...
VIDEO_OPTS="-vcodec libx264 -f mpegts -movflags faststart -crf 28 -s 640x360 -r 5"

completed="INCOMPLETE"
while true; do
    if [ "$STREAM_URL" != " " ] && [ "$completed" == "INCOMPLETE" ]; then
        ffmpeg -i ${STREAM_URL}?pkt_size=18800 ${GENERAL_OPTS} ${VIDEO_OPTS} -force_key_frames "expr:gte(t,n_forced*${time_segment_s})" ${SEGMENT_OPTS}
